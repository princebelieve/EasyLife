...
router.get("/product/:id", async (req, res) => {
  try {
    ...
    sharePage(res, { title: `${product.name} | EASYLIFE WELLNESS HUB`, description: product.fullDescription || product.shortDescription || `Shop ${product.name} from EASYLIFE WELLNESS HUB.`, image: product.coverImage, canonicalUrl: productUrl, redirectUrl: productUrl });
  } catch (error) {
...
router.get("/content/:id", async (req, res) => {
  try {
    ...
    sharePage(res, { title: post.title || post.name || "EASYLIFE update", description: post.seoDescription || post.testimony || "Read the latest EASYLIFE WELLNESS HUB update.", image: post.image || youtubeThumbnail(post.videoUrl), canonicalUrl: postUrl, redirectUrl: postUrl });
  } catch (error) {
