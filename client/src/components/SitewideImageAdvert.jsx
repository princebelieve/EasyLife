...
export default function SitewideImageAdvert() {
  ...
  const content = <><img src={advert.image} alt={advert.title || advert.name || "EASYLIFE promotion"} /><div className="sitewide-image-advert-copy"><strong>{advert.title || advert.name}</strong><span>{advert.testimony}</span></div></>;
  return <aside className="sitewide-image-advert" aria-label="EASYLIFE promotion"><button type="button" className="sitewide-image-advert-close" onClick={close} aria-label="Dismiss promotion"><X size={17} /></button>{/^https?:\/\//i.test(target) ? <a href={target} target="_blank" rel="noreferrer">{content}</a> : <Link to={target}>{content}</Link>}{adverts.length > 1 && <div className="sitewide-image-advert-dots">{adverts.map((item, itemIndex) => <button key={item._id} type="button" className={itemIndex === index ? "active" : ""} onClick={() => setIndex(itemIndex)} aria-label={`Show advert ${itemIndex + 1}`} />)}</div>}</aside>;
}
