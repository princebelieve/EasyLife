...
export default function AdminPaymentSettings() {
  ...
  return <div className="page"><h1>Store Payment Settings</h1><p className="muted">Choose the bank and enter the 10-digit account number. Easy Life verifies the account name before saving; customer transfers still go directly to your bank.</p><form className="form" onSubmit={save}><label className="wizard-checkbox"><input type="checkbox" name="manualTransferEnabled" checked={settings.manualTransferEnabled} onChange={change} /><span>Enable manual bank transfer at main checkout</span></label><select required={settings.manualTransferEnabled} name="bankCode" value={settings.bankCode} onChange={change}><option value="">Select Nigerian bank</option>{banks.map((bank) => <option key={bank.code} value={bank.code}>{bank.name}</option>)}</select><input required={settings.manualTransferEnabled} inputMode="numeric" name="accountNumber" placeholder="10-digit account number" value={settings.accountNumber} onChange={change} /><button type="button" className="secondary-button" onClick={resolveAccount} disabled={resolving || !settings.bankCode || settings.accountNumber.length !== 10}>{resolving ? "Verifying account…" : "Verify account name"}</button><input readOnly name="accountName" placeholder="Verified account name appears here" value={settings.accountName} /><textarea name="transferInstructions" rows="4" placeholder="Optional instructions, e.g. use order number as transfer narration" value={settings.transferInstructions} onChange={change} /><button className="primary" type="submit" disabled={settings.manualTransferEnabled && !settings.accountName}>Save payment settings</button>{message && <p className="inline-toast success">{message}</p>}</form></div>;
}
