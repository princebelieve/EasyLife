  ...
  return (
    <>
      ...
      <main className="easy-life-home">
        ...
        <section className="director-home-feature"><div className="container director-home-feature-grid"><div className="director-home-portrait"><span>MEET OUR DIRECTOR</span><img src="/ceo.png" alt="Dr. Comfort Chinaza Nwaka (Dr. Chizzy Lisa), licensed optometrist and Director of Easy Life Wellness Hub" /></div><div className="director-home-copy"><p className="easy-eyebrow">LEADERSHIP WITH HEART</p><h2>Meet Dr. Comfort Chinaza Nwaka.</h2><p>Also known as Dr. Chizzy Lisa, she is the licensed optometrist leading Easy Life Wellness Hub, Easy Life Clinic, and Easy Life Supermarket. With her team, she brings wellness products, practical education, and community support together in one place.</p><Link to="/our-director" className="easy-btn easy-btn-primary">Meet our Director <ArrowRight size={18} /></Link></div></div></section>
        ...
        <section className="section easy-training-section reveal"><div className="container easy-two-column"><div><p className="easy-eyebrow">FREE WEEKLY TRAINING</p><h2>Learn practical wellness, hygiene, and community skills.</h2><p>Registered members can attend free training every Thursday at 11:00 AM at our Akpakpava location. Sessions cover wellness and hygiene education, responsible product use according to label directions, leadership, and community outreach.</p><Link to={isLoggedIn ? "/dashboard" : "/register"} className="easy-btn easy-btn-primary breathing-button">{isLoggedIn ? "Go to your dashboard" : "Join the community"} <ArrowRight size={18} /></Link><ul className="easy-training-list">{trainings.map((item) => <li key={item}>✓ {item}</li>)}</ul></div><img className="easy-section-image" src="/image-5.png" alt="Easy Life wellness education and community training" /></div></section>

        {testimonials.length > 0 && <section className="section testimonials-showcase reveal"><div className="container"><div className="easy-section-heading easy-heading-row"><div><p className="easy-eyebrow">COMMUNITY STORIES</p><h2>See how the Easy Life community is growing.</h2></div><Link className="easy-text-link" to="/testimonials">View all stories <ArrowRight size={17} /></Link></div><div className="testimonial-home-grid">{testimonials.map((item) => { const embedUrl = getVideoEmbedUrl(item.videoUrl); const isVideoStory = Boolean(item.videoFile || embedUrl); return <article className={`testimonial-home-card content-card${isVideoStory ? " video-story-card" : ""}`} key={item._id}>{(item.videoFile || item.audioFile || embedUrl || item.image) && <div className="testimonial-home-media">{item.videoFile ? <video controls preload={item.image ? "none" : "metadata"} poster={item.image || undefined} src={item.videoFile} /> : item.audioFile ? <audio controls preload="metadata" src={item.audioFile} /> : embedUrl ? <iframe title={`${item.name} testimonial video`} src={embedUrl} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : <img src={item.image} alt={item.name} />}</div>}<div className="testimonial-home-body">{isVideoStory && item.title && <h3 className="video-story-title">{item.title}</h3>}<p>“{item.testimony}”</p><strong>{item.name}</strong>{item.role && <span>{item.role}</span>}<PostShareButton title={item.title || item.name} text={item.testimony} url={`${window.location.origin}/testimonials#${item._id}`} /></div></article>; })}</div></div></section>}

        <section className="easy-membership"><div className="container easy-membership-grid"><div><p className="easy-eyebrow">MEMBERSHIP</p><h2>Learn freely. Participate fully.</h2><p>Registered members receive free Thursday training at 11:00 AM in Akpakpava, wellness education, leadership development, and opportunities to join official health-awareness outreaches.</p></div><div className="easy-member-card content-card"><strong>Registered members can</strong><ul><li>Attend free weekly wellness and hygiene training</li><li>Learn responsible product use according to label directions</li><li>Follow Easy Life to official health-awareness outreaches</li><li>Build confidence, leadership, and community connections</li></ul><Link to={isLoggedIn ? "/dashboard" : "/register"} className="easy-btn easy-btn-light breathing-button">{isLoggedIn ? "Open your dashboard" : "Become a member"}</Link></div></div></section>
        ...
        <section className="easy-closing reveal"><div className="container"><p className="easy-eyebrow">WELLNESS STARTS WITH KNOWLEDGE</p><h2>We help you learn and live well.</h2><p className="easy-closing-copy">Live the Easy Life.<br />Healthy body. Strong mind. Supportive community.</p><div className="easy-actions"><Link className="easy-btn easy-btn-primary breathing-button" to={isLoggedIn ? "/dashboard" : "/register"}>{isLoggedIn ? "Go to your dashboard" : "Join the community"}</Link><Link className="easy-btn easy-btn-light breathing-button" to="/contact">Contact Easy Life</Link></div></div></section>
      </main>
      <Footer />
    </>
  );
