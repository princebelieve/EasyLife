  ...
  return (
    <>
      ...
      <main className="easy-life-home">
        ...
        <section className="section easy-services reveal"><div className="container easy-two-column"><div><p className="easy-eyebrow">TEST & CHECKUP</p><h2>Know more. Prevent earlier. Live stronger.</h2><p>Accurate wellness testing and checkups support early detection and prevention. Explore wellness education, product guidance and available appointments with an Easy Life representative.</p><Link to="/contact" className="easy-btn easy-btn-primary">Make an enquiry <ArrowRight size={18} /></Link></div><div><div className="easy-service-list"><div><strong>Naturopathic wellness</strong><span>Natural solutions for a healthier, longer, vibrant life.</span></div><div><strong>Naturopathic equipment</strong><span>High-quality wellness equipment for home use and professional care.</span></div><div><strong>Test & checkup</strong><span>Accurate wellness testing and checkups for early detection and prevention.</span></div></div><img className="easy-section-image" src="/image-3.png" alt="Professional wellness testing and checkup session" /></div></div></section>

        <section className="section easy-training-section reveal"><div className="container easy-two-column"><div><p className="easy-eyebrow">NETWORK MARKETING</p><h2>A proven platform to build your team and earn unlimited income.</h2><p>Live better, earn big, improve lives, create impact, and enjoy financial independence through practical training and ethical opportunity.</p><Link to="/register" className="easy-btn easy-btn-primary">Join the movement <ArrowRight size={18} /></Link><ul className="easy-training-list">{trainings.map((item) => <li key={item}>✓ {item}</li>)}</ul></div><img className="easy-section-image" src="/image-5.png" alt="Easy Life leadership and team building training" /></div></section>
